...
function App() {
  ...
  const handlePxChange = useCallback(
    (val: string) => {
      ...
      setRemValue(!isNaN(n) && baseFontSize > 0 ? String(round(n / baseFontSize)) : '')
    },
    [baseFontSize]
  )
  ...
  const handleRemChange = useCallback(
    (val: string) => {
      ...
      setActiveField('rem')
      const n = parseFloat(val)
      setPxValue(!isNaN(n) && baseFontSize > 0 ? String(round(n * baseFontSize)) : '')
    },
    [baseFontSize]
  )
  ...
  const handleSwap = () => {
    ...
    // Keep the number in the LEFT field but reinterpret it with the opposite unit
    const leftValue = swapped ? remValue : pxValue
    const n = parseFloat(leftValue)
    if (!isNaN(n) && baseFontSize > 0) {
      if (nextSwapped) {
        // left becomes REM: treat leftValue as rem, recalculate px
        setRemValue(leftValue)
        setPxValue(String(round(n * baseFontSize)))
        setActiveField('rem')
      } else {
        // left becomes PX: treat leftValue as px, recalculate rem
  ...
  const pxSteps = [4, 8, 10, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 96, 112, 128]
  ...
  return (
    <main className="app">
      ...
      <header className="page-header">
        <div>
          ...
          <p className="subtitle">
            {swapped ? 'Converting rem to pixel units' : 'Converting pixels to rem units'}
          </p>
        </div>
        <button
      ...
      <section className="converter" aria-label="Converter">
        {/* Left field — role depends on swapped */}
        <div className="field">
          ...
          <input
            ...
            onChange={e => swapped ? handleRemChange(e.target.value) : handlePxChange(e.target.value)}
          />
          <div className="field-meta">
            <span className="unit-label">{swapped ? 'rem' : 'px'}</span>
            <button
              className={`copy-btn${copiedField === (swapped ? 'rem' : 'px') ? ' is-copied' : ''}`}
              disabled={swapped ? !remValue : !pxValue}
              onClick={() => copy(swapped ? remValue : pxValue, swapped ? 'rem' : 'px')}
            >
        ...
        {/* Right field — role depends on swapped */}
        <div className="field">
          ...
          <input
            ...
            onChange={e => swapped ? handlePxChange(e.target.value) : handleRemChange(e.target.value)}
          />
          <div className="field-meta">
            <span className="unit-label">{swapped ? 'px' : 'rem'}</span>
            <button
              className={`copy-btn${copiedField === (swapped ? 'px' : 'rem') ? ' is-copied' : ''}`}
              disabled={swapped ? !pxValue : !remValue}
              onClick={() => copy(swapped ? pxValue : remValue, swapped ? 'px' : 'rem')}
            >
          ...
          <tbody>
            {pxSteps.map(px => {
              ...
              const handleRowClick = () =>
                swapped ? handleRemChange(String(rem)) : handlePxChange(String(px))
              return (
                <tr
                  key={px}
                  ...
                  aria-label={swapped ? `${rem}rem = ${px}px` : `${px}px = ${rem}rem`}
                >
